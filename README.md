...
* Description: Ridership data contains stop-level boarding, alighting, and load observations across several years and seasons from 2016 to 2024. Arrival–Departure time data stores detailed service-level information such as headways, scheduled times, and earliness or lateness of bus arrivals for each route and direction.
* Source: Kaggle dataset above.

**3. Census Tract Shapefiles:**
...
**4. ACS Race Demographics (Table B03002):**
...
### III. Route-Level Clustering and Equity Patterns

To understand whether reliability inequities show up as distinct “regimes” of service, we perform both **K-Means** and **hierarchical clustering** on route-level summaries. Importantly, **race is never used as an input feature to the clustering algorithms**; demographic variables are only used afterwards to interpret equity patterns.

#### 1. K-Means clustering on lateness vs. % non-White

We first cluster routes in a simple 2-D space with:

* (x): average % non-White riders along the route
* (y): average lateness (minutes)

This gives an easily interpretable view of how delay and race co-vary across routes.

<br>

<img width="600" alt="K-Means clusters of routes by lateness and % non-white" src="src/p1.png" />

<br>

Each color corresponds to a K-Means cluster. We observe that routes with **both higher lateness and higher % non-White** tend to fall into the same clusters, suggesting that the worst delay patterns are concentrated on corridors that serve riders of color.

#### 2. Correlation-based hierarchical clustering of route profiles

Next, we construct a richer route-level feature matrix including:

* lateness mean and variability,
* scheduled and observed headways,
* on-time performance rate,
* stop count and ridership intensity.

We compute a **1 − correlation** distance between routes and apply **agglomerative hierarchical clustering** to group routes with similar operational signatures.

<br>

<img width="900" alt="Hierarchical clustering of routes (1 - correlation)" src="src/p2.png" />

<br>

To select the number of clusters, we evaluate the **silhouette score** for different values of (k). A clear peak around 3–4 clusters supports the choice used in the analysis.

<br>

<img width="500" alt="Silhouette score vs k (hierarchical clustering)" src="src/p3.png" />

<br>

#### 3. Cluster profiles: demographics and lateness

Once clusters are assigned, we compute cluster-level averages of:

* `pct_nonwhite_route`,
* `pct_white_route`,
* `lateness_mean`.

Two visual summaries capture these relationships:

<br>

<img width="450" alt="Cluster-level averages (heatmap)" src="src/p4.png" />

<br>

<img width="600" alt="Cluster profiles across demographics and lateness" src="src/p5.png" />

<br>

Interpretation:

* One cluster combines **high lateness** with **high % non-White** and lower % White.
* Another cluster has **lower lateness** and a higher share of White riders.
* A middle cluster shows moderate lateness and mixed demographics.

We also examine which clusters contain the **worst-lateness routes** (e.g., routes above the 80th percentile of lateness):

<br>

<img width="500" alt="Share of worst-lateness routes by cluster" src="src/p9.png" />

<br>

The majority of extreme-lateness routes fall into a single correlation-based cluster, reinforcing that poor performance is not random but concentrated in specific service regimes.

#### 4. Spatial clustering and geographic patterns

To understand where these clustered service regimes live in space, we also:

* cluster routes based on **geographic centroids and shapes**,
* project route features into **PCA space** for visualization, and
* map each route polyline colored by its hierarchical “equity cluster.”

<br>

<img width="900" alt="Hierarchical clustering of routes (geographic centroids)" src="src/p6.png" />

<br>

<img width="700" alt="Routes in PCA space, colored by cluster" src="src/p7.png" />

<br>

These spatial views show that the cluster with **higher lateness and higher % non-White exposure** is concentrated in specific corridors, especially in neighborhoods with larger shares of riders of color. Together, the clustering results provide a complementary, unsupervised view that supports the equity findings from the descriptive analysis and predictive modeling.

---

### IV. Lateness Prediction: XGBoost + SHAP Explainability
